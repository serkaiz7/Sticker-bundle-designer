...
export class CanvasManager {
    ...
    initStage(width, height) {
        ...
        // Use IDs for reliable selection after deserialization
        this.layer = new Konva.Layer({ id: 'main-layer' });
        ...
        // Attach stage-level event listeners
        this.attachStageListeners();
        this.saveState();
    }

    /**
     * Attaches the necessary event listeners to the main stage.
     * This is separated to be called after loading a state.
     */
    ...
    setCanvasSize(sizeKey) {
        ...
        let width = size.isPx ? size.width : Math.round(size.width * this.DPI);
        let height = size.isPx ? size.height : Math.round(size.height * this.DPI);
    ...
    addPreset(type) {
        let preset;
        const fill = document.body.classList.contains('dark') ? '#555' : '#ddd';
        const stroke = document.body.classList.contains('dark') ? '#999' : '#555';
        if (type === 'rectangle') { preset = new Konva.Rect({ x: 50, y: 50, width: 200, height: 100, fill, stroke, strokeWidth: 2, draggable: true }); } 
        else if (type === 'circle') { preset = new Konva.Circle({ x: 150, y: 150, radius: 70, fill, stroke, strokeWidth: 2, draggable: true }); }
    ...
    handleSelect(e) {
        ...
        // Find parent group if it's draggable, otherwise select the shape
        let target = e.target;
        while (target.parent && !(target instanceof Konva.Group && target.draggable()) && target.parent !== this.layer) {
        ...
        if (!metaPressed && !isSelected) { this.tr.nodes([target]); } 
        else if (metaPressed && isSelected) { const nodes = this.tr.nodes().slice(); nodes.splice(nodes.indexOf(target), 1); this.tr.nodes(nodes); } 
        else if (metaPressed && !isSelected) { const nodes = this.tr.nodes().concat([target]); this.tr.nodes(nodes); }
    ...
    loadState(jsonString) {
        ...
        // Re-assign core component references using IDs
        this.gridLayer = this.stage.findOne('#grid-layer');
        this.layer = this.stage.findOne('#main-layer');

        // Re-create and add the transformer because it's not serialized
        this.tr = new Konva.Transformer({ anchorStroke: '#4a90e2', anchorFill: 'white', anchorSize: 10, borderStroke: '#4a90e2', borderDash: [4, 4], rotateAnchorOffset: 30, keepRatio: false });
        this.layer.add(this.tr);
    ...
    group() {
        if (this.selectedNodes.length > 1) {
            ...
            const box = Konva.Util.getClientRect(this.selectedNodes);
            group.position({ x: box.x, y: box.y });
            this.selectedNodes.forEach(node => { 
                node.moveTo(group); 
                node.position({ x: node.x() - box.x, y: node.y() - box.y }); 
                // Ensure nodes are not individually draggable inside a group
    ...
    ungroup() {
        if (this.selectedNodes.length === 1 && this.selectedNodes[0] instanceof Konva.Group) {
            ...
            children.forEach(child => { 
                const pos = child.getAbsolutePosition(this.stage);
                child.moveTo(this.layer); 
                child.position(pos); 
                child.draggable(true); // Make nodes draggable again
            });
            group.destroy(); 
            this.tr.nodes(children); 
