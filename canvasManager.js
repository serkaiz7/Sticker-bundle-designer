// canvasManager.js
// ES module
// Usage: import { CanvasManager } from './canvasManager.js';
...
export class CanvasManager {
  ...
  _initStage() {
    ...
    // ensure container has explicit size (CSS may resize later)
    ...
    // track container for fit/centering
    this.containerEl = container.parentElement || container;
  }

  _initLayers() {
    // background (paper)
    this.bgLayer = new Konva.Layer({ id: 'bgLayer', listening: false });
    this.paper = new Konva.Rect({
      x: 0, y: 0, width: this.A4.w, height: this.A4.h, fill: '#ffffff', cornerRadius: 6
    });
    this.bgLayer.add(this.paper);
    // grid & guides
    this.gridLayer = new Konva.Layer({ id: 'gridLayer', listening: false, visible: false });
    // content (user objects)
    this.contentLayer = new Konva.Layer({ id: 'contentLayer' });
    // guides
    this.guideLayer = new Konva.Layer({ id: 'guideLayer', listening: false });
  ...
  /* ---------------- events: pan & zoom & keyboard ---------------- */

  _bindStageEvents() {
    // center/fill artboard
    window.addEventListener('resize', () => this.fitToContainer());
    this.fitToContainer();

    // mouse wheel zoom — require ctrl/meta for zoom to avoid interfering with scroll
    this.stage.on('wheel', (e) => this._onWheel(e));

    // spacebar pan: track space key globally
    window.addEventListener('keydown', (e) => {
      if (e.code === 'Space' && !this._spacePressed) {
        this._spacePressed = true;
        this.stage.container().style.cursor = 'grab';
      }
      // ctrl/cmd + 0 / + / - handled here
    ...
    // implement panning while space pressed
    ...
    // deselect transformer when clicking outside objects
  ...
  _onWheel(e) {
    // if ctrl/meta pressed OR (optional) always allow wheel zoom: prefer ctrl/meta to be safe
    const evt = e.evt;
  ...
  fitToContainer(instant = false) {
    // Scale stage so A4 fits into container element while preserving aspect ratio.
    const parent = this.containerEl;
    const pw = parent.clientWidth - 48; // some padding
    const ph = parent.clientHeight - 120;
    const scale = Math.min(pw / this.A4.w, ph / this.A4.h, 1);
    // set stage absolute size (keeps internal width/height unchanged)
  ...
  _setupObject(node, isText = false) {
    ...
    node.on('mousedown touchstart', (e) => {
      // prevent panning if interacting with object
      e.cancelBubble = true;
    });
    node.on('dragmove', () => this._onNodeDragMove(node));
    node.on('dragend', () => {
      // when drag ends, try to insert into a preset
      const inserted = this.tryInsertIntoPreset(node);
      if (!inserted) {
        // if wasn't inserted but it was inside a group previously and moved out, ensure it's on content layer
        // (leave as-is)
      }
      this._commitHistory();
    });
    ...
    // double-click for text inline editing can be handled externally by app
    this.contentLayer.draw();
  }

  _onNodeDragMove(node) {
    // optional snapping to guides / canvas center
    // show guideline if near centers
    this._clearGuides();
  ...
  _createRectPreset(x, y, w, h) {
    ...
    const outline = new Konva.Rect({ x: 0, y: 0, width: w, height: h, fillEnabled: false, stroke: '#9ca3af', dash: [6,6], strokeWidth: 1, listening:false });
    group.add(outline);
  ...
  _createCirclePreset(cx, cy, r) {
    ...
    const outline = new Konva.Circle({ x: r, y: r, radius: r, fillEnabled: false, stroke: '#9ca3af', dash: [6,6], strokeWidth: 1, listening:false });
    group.add(outline);
  ...
  /* ---------------- insertion into preset (safe, non-vanishing) ---------------- */

  tryInsertIntoPreset(node) {
    // when node is dragged or placed, detect intersection with presets (absolute coords)
    for (const group of this._presets.slice().reverse()) {
      ...
      if (Konva.Util.haveIntersection(presetAbs, nodeAbs)) {
        // compute absolute position of node top-left
        const nodeAbsPos = node.getAbsolutePosition();
        const groupAbsPos = group.getAbsolutePosition();

        // detach node into group coordinate space
        // IMPORTANT: preserve image's underlying image object & size
        node.moveTo(group);
        // set node pos relative to group's coordinate space
        node.position({ x: nodeAbsPos.x - groupAbsPos.x, y: nodeAbsPos.y - groupAbsPos.y });

        // Fit (cover) the node to group's bounds
        ...
        // ensure node interactive
        node.draggable(true);
        node.setAttr('selectable', true);

        // select node with transformer
        this.transformer.nodes([node]);
        this.contentLayer.draw();
        this._commitHistory();
        return true;
  ...
  _fitNodeToBounds(node, w, h) {
    // uses cover behavior: scale node so it covers w x h box
    ...
    // center inside group bounds
    node.x((w - node.width()) / 2);
    node.y((h - node.height()) / 2);
  }

  /* ---------------- clipboard / selection / transform helpers ---------------- */
  ...
  duplicateSelected() {
    ...
    nodes.forEach(n => {
      ...
      // ensure selectable on cloned
      clone.setAttr && clone.setAttr('selectable', true);
      this._setupObject(clone, clone.getClassName() === 'Text');
      // if cloned inside a group, move to same parent
  ...
  pasteClipboard() {
    ...
    this._clipboard.forEach(j => {
      ...
      // offset pasted nodes
      node.x((node.x() || 0) + 12); node.y((node.y() || 0) + 12);
  ...
  groupSelected() {
    ...
    // compute group's absolute pos as min of nodes
  ...
  ungroupSelected() {
    ...
    nodes.forEach(n => {
      if (n.getClassName() === 'Group') {
        ...
        const abs = n.getAbsolutePosition();
        children.forEach(c => {
          const childAbs = c.getAbsolutePosition();
          c.moveTo(this.contentLayer);
          c.position({ x: childAbs.x, y: childAbs.y });
        });
        n.destroy();
      }
    });
  ...
  async toPDF() {
    ...
    const img = this.toDataURL(this.pixelRatioExport);
  ...
  /* ---------------- history (content-layer only) ---------------- */

  _commitHistory() {
    // throttle commits to animation frame
  ...
  undo() {
    if (this._historyPos > 0) {
      this._historyPos--;
      this._restoreHistoryAt(this._historyPos);
    }
  }
  ...
  _restoreHistoryAt(pos) {
    ...
    if (!snapshot) return;
    try {
      // clear current content layer
      this.contentLayer.destroyChildren();
      const restoredLayer = Konva.Node.create(snapshot);
      // restoredLayer might be a Layer; iterate children and add them
      ...
      // ensure transformer is present on contentLayer
      this.contentLayer.add(this.transformer);
      this.contentLayer.draw();
      this.onHistory(snapshot);
      window.dispatchEvent(new CustomEvent('canvas:history', { detail: { snapshot } }));
  ...
  _emitSelectionChange(node) {
    // application UI can listen to selection events if needed
